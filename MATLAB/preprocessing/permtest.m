...
samplediff = abs(nanmean(sample1) - nanmean(sample2)); % Calculate the difference between the means of the samples
...
for n = 1:numperm
    ...
    sampdiffshuff(n) = abs(nanmean(sampshuff(1:length(sampshuff)/2)) - nanmean(sampshuff(length(sampshuff)/2+1:end))); % Calculate the difference between means for the shuffled samples
end
